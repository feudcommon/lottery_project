...
const buyTicket = asyncHandler(async (req, res) => {
  ...
  if (slotNumber === undefined || slotNumber === null) {
    ...
    const soldRows = await db.prepare("SELECT ticket_number FROM tickets WHERE draw_date = ?")
      .all(drawDate_str);
  ...
  const result = await ticketService.buyTicket(userId, drawDate, slotNumber);
...
const getMyTicketsToday = asyncHandler(async (req, res) => {
  ...
  // Get user's tickets
  const myTickets = await ticketService.getMyTicketsForDate(req.user.id, drawDate);
  ...
  const available = await ticketService.getAvailableTicketCount(drawDate);
